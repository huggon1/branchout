...
Run `npm run build:icons` on macOS to render the SVG with the pinned Electron runtime and create PNG, ICNS and ICO outputs. Commit the SVG, PNG, ICNS and ICO; the intermediate iconset is ignored. `npm run build` copies runtime PNGs into `dist/assets`, and packaging uses the native ICNS or ICO asset for its target platform.
