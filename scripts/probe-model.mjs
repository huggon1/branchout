...
const dataDir = await mkdtemp(join(tmpdir(), 'feedloom-model-'));
configureNetwork();
let streamed = false;
try {
  const result = await generateText({ text: 'Fictional integration test.', instruction: 'Reply exactly FEEDLOOM_OK.', dataDir, onProgress: () => { streamed = true; } });
  const success = result.text === 'FEEDLOOM_OK' && result.tools.length === 0;
