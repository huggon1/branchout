...
Codex 会话候选记录会话身份、时间、工作目录、可核验的项目归属线索与选入状态。选中的会话读取器交付经固定代码解析的用户发言及必要的最终助手回复，每条片段保存角色、会话身份、消息定位和正文；解析覆盖量与读取失败情况进入报告覆盖范围。项目分析证据 `AnalysisEvidenceRef` 标明来源类别 `repository`、`commit` 或 `codex_session`，以及相应的版本标识、文件或消息位置和可读摘录。
