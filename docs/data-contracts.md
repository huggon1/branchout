...
Codex 会话候选记录会话身份、时间、工作目录、可核验的项目归属线索与选入状态。选中的会话读取器交付带会话和消息定位的片段；归属依据及读取失败情况进入报告覆盖范围。项目分析证据 `AnalysisEvidenceRef` 标明来源类别 `repository`、`commit` 或 `codex_session`，以及相应的版本标识、文件或消息位置和可读摘录。
