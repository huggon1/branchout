...
仓库分析复用 Pi SDK 的多轮执行、工具事件与压缩，仅允许固定版本目录与正文读取工具；不开通 Shell、写入或仓库内插件执行。模型输出引用工具返回的来源 ID，业务层解析为真实版本 URL 并逐字核验。读取预算和模型轮次有界，不持久化内部思维。
...
| 仓库理解版本    | 产品定位、使用情境、必要边界、文件/PR/commit 依据、生成时间；保存后不可变            |
...
记录研究问题覆盖、证据缺口和新增相关来源，用于解释继续或停止。模型停止提议由核心校验是否存在对应观察；不接受没有执行记录的“充分”。已有可用素材、启用平台均已尝试且连续两次正常搜索没有新增来源时，执行器可以停止重复搜索，保留来源未穷尽的说明。执行上限见 PRD，停止质量继续用真实样例校准，不设置最低素材产出要求。
