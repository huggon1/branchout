...
个人试用优先验证 Codex OAuth 与 `gpt-5.6-luna`，同时支持用户配置 API Key。凭据接入必须验证刷新与并发所有权，不让应用与 Codex 各自刷新同一凭据副本。无法复用时提供独立登录；不自动切换模型或计费接口。首版不要求本地模型，也不提供统一模型额度服务。
...
2. 调度器创建收集记录，调用采集适配器。优先复用 last30days 的可验证采集入口；GitHub Trending、平台直接接口和本机服务均经适配器接入。验证原始输出、时间窗口和缺失指标；外部重排或聚合行为不能直接变成 Feedloom 的业务规则。
3. 按明确规则筛选并写入素材库；模型只生成摘要，不隐藏素材、不合并来源、不降低门槛凑足数量。每轮保留平台状态与素材关系。
...
首版筛选、排序和选材行为以 PRD 为准，不额外引入素材库关键词搜索、向量库或语义检索。应用更新涉及数据结构变化时执行版本化迁移，验证旧数据可继续使用；首版不建设手动备份、导出和恢复功能。
