...
项目分析任务先建立本机仓库输入快照，记录仓库目录、Git HEAD、工作区状态、实际读取的文件和 commit 范围。Codex 会话读取器从本机索引和会话元数据发现候选，依据工作目录与 Git 仓库身份判断归属；用户在提交前确认待选会话。确定性的会话解析器只交付用户发言和必要的最终助手回复及其消息定位，模型思考、工具调用与输出、系统配置及凭据由解析器过滤。模型输入使用解析后的对话片段。
