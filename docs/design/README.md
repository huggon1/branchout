...
全局空、收藏夹空、筛选无匹配分别说明下一步；解析中、部分解析、正文失败、摘要失败、中断可重试、机器人待整理与提示过期均以文字和语义状态呈现。机器人聊天先用短句确认保存位置与数量，再提供纯文本序号／完整名称和 `新建 收藏夹名称`；无效、过期、已完成、平台失败与移动成功分别说明结果及可用下一步，不依赖卡片或按钮。删除非空收藏夹明确说明内容将移至当前默认收藏夹。创建、重命名和管理操作沿用现有输入、弹出菜单、确认与焦点规则。
