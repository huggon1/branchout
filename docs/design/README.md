...
| 工作空间   | 内部页面与衔接                                                                 |
| ---------- | ------------------------------------------------------------------------------ |
| 内容收集   | 收藏与阅读：整理粘贴/转发内容，查看解析状态、原文和摘要；不进入素材库          |
| 项目理解   | 项目概览与进展：选择本机 Git 目录，手动分析，展示固定版本、依据及近期变化历史   |
| 素材探索   | 新建探索、持久运行详情、素材库和只读历史收集；从理解到发现再到人工选材          |
| Feed 创作  | 生成 Feed 与 Feed 历史；核对来源快照、章节、顺序和提示词，再阅读与追溯成品      |
| 连接与模型 | 独立设置入口；配置公开来源能力、模型及 Telegram／飞书机器人，不是业务工作空间   |

侧栏只保留四个顶层工作空间，当前工作空间在原位展开直接子页面。主区标题上方显示“工作空间 / 当前页面”，标题下方用一条轻量行动带说明真实资产、当前缺失、下一步和直接恢复入口。顶层切换回到该工作空间上次停留的页面；运行详情、项目证据、内容详情和 Feed 历史仍由各自领域页面承载，不复制成壳层概览。行动带不得把缺失依赖做成无解释的禁用按钮，也不得自动代办分析或探索。

项目理解以系统目录选择器添加本机 Git 项目，卡片不展示绝对路径；分析失败保留上次成功理解，无可用理解显示原因。旧 GitHub 项目标为只读并提供“重新关联本机目录”，不能伪装成仍可分析。当前 checkout 区显示分支和缩写 commit；分支变化须确认，detached HEAD 有独立提示。探索中的版本卡显示“理解版本 / 生成时间 / 仓库引用 / commit”，不标“最新”，不自动触发分析，也不提供理解纠正或模板编辑入口。
...
全局空、收藏夹空、筛选无匹配分别说明下一步；解析中、部分解析、正文失败、摘要失败和中断可重试均以文字和语义状态呈现。机器人转发保存到当时的默认收藏夹；创建、重命名、移动和删除收藏夹只在桌面完成，并沿用现有输入、弹出菜单、确认与焦点规则。删除非空收藏夹明确说明内容将移至当前默认收藏夹。
...
- 项目理解、素材探索和内容收集可独立进入；探索缺理解时定位到项目理解，用户自行发起分析。
