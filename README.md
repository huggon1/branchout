...
数据库按顺序升级至 v5：v4 为探索批次和组合迁移独立生命周期、结果、停止原因、进度、事实事件与遥测，v5 增加主进程私有的本机目录绑定并清理已停用的项目仓库凭据。旧仓库、理解、分析、探索、素材、运行和 Feed 快照保持原样；旧任务的 `CollectionBudget` 继续可读。服务商未报告 token 时界面明确显示不可用，不用 0 代替。
