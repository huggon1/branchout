...
- **探索运行**：配置提交后立即保存批次并进入独立详情页，查看任务队列、阶段、当前行动、有效进展、证据缺口、下一步依据、覆盖情况与真实用量。可暂停／继续整批或单项；同批其他任务仍在运行时，已完成结果即可阅读。Agent 根据已读结果改词或补读，候选必须有可核验的原文与近期活动证据才能入库；平台限制不当作无结果。
...
数据库升级至 v4，为探索批次和组合迁移独立生命周期、结果、停止原因、进度、事实事件与遥测；旧任务的 `CollectionBudget`、历史收集配置、运行和 Feed 快照保持原样。服务商未报告 token 时界面明确显示不可用，不用 0 代替。
