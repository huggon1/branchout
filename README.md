...
- **内容收集**：粘贴分享文案或通过 Telegram／飞书机器人接收本人私聊的 GitHub 仓库、小红书链接。每条消息最多处理十个不同链接，作为同一个整理批次原子保存到当前默认收藏夹；平台重复投递不重复入库。机器人确认保存后给出纯文本收藏夹菜单，回复该消息的序号或完整名称可移动整批，回复 `新建 收藏夹名称` 可创建并移动；七天不回复时内容仍安全留在默认收藏夹。整理会话、过期与完成状态跨重启保留，重复回复不会重复创建或移动。初始默认收藏夹为 `Inbox`，可创建、重命名、更换默认、单条或批量移动；删除非空收藏夹前会确认并把内容原子移动到当前默认收藏夹。阅读 README／正文、图片与 AI 摘要，正文和摘要状态分开展示，独立保存、不进入素材库或 Feed。README 按实际路径和可获取的 commit 解析相对图片与链接，常见 HTML 结构经过白名单清洗，禁用脚本；图片可以预览，加载失败独立提示。
- **项目回顾**：从本机选择已有 Git 工作目录，手动运行分析。每次运行固定当前分支和 commit，只读取该 commit 的 Git 对象，不读取未提交或未跟踪文件、符号链接目标及 submodule 内容。产品理解带版本、生成时间及固定 commit；近期变化分析独立呈现。首次整理最近 7 天，之后增量追加开发时间线；概览与变化分别完成，未完成的变化不推进边界。
...
数据库按顺序升级至 v7：v4 为探索批次和组合迁移独立生命周期、结果、停止原因、进度、事实事件与遥测；v5 增加主进程私有的本机目录绑定并清理已停用的项目仓库凭据；v6 新增收藏夹与内容分配表，创建默认 `Inbox`，只按旧记录 ID 建立分配；v7 在其后新增机器人整理会话和回复回执索引。v6、v7 都不改写原有转发记录的来源 URL、渠道、正文、媒体、摘要、错误或状态。旧仓库、理解、分析、探索、素材、运行和 Feed 快照保持原样；旧任务的 `CollectionBudget` 继续可读。服务商未报告 token 时界面明确显示不可用，不用 0 代替。
