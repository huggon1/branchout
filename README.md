...
- **项目回顾**：绑定 GitHub 公开或已授权私有仓库，手动运行分析。产品理解带版本、生成时间及固定 commit；近期变化分析独立呈现。首次回顾最近 7 天，之后使用成功分析边界；失败不推进边界。
...
仓库分析限定默认分支与关联已合并 PR，最多读取 25 个增量 commits 和 16 个上下文文件，API 请求与模型输入均有上限。目录或变更范围无法可靠读取时显示失败且不推进边界；长文件与 patch 截断在理解的信息缺口中标明。
...
Pi 在后台进程中运行，使用内存会话并禁用任意工具、扩展、Skills 和上下文文件自动发现。业务执行器掌握循环、预算与取消，模型只提出结构化动作和判断。仓库和来源文本均不具备指令权限，摘录须逐字存在于对应输入。私有仓库分析可向配置的模型发送必要内容；对公开平台检索仅发送抽象需求与场景，并阻止包含仓库标识、代码或凭据形态的搜索词。外部内容采用清洗后的 Markdown／HTML 渲染，外链仅允许不含凭据的公开 HTTPS 地址。
