...
app.whenReady().then(async () => {
  ...
  win = new BrowserWindow({ width: 1060, height: 780, title: 'Feedloom · 连接 X', webPreferences: { session: isolated, nodeIntegration: false, contextIsolation: true, sandbox: true } });
  ...
  timer = setInterval(async () => {
    ...
    if (cookies.some(c => c.name === 'auth_token') && cookies.some(c => c.name === 'ct0')) {
      ...
      win.setTitle('Feedloom · X 登录已保存，待验证搜索');
    }
  }, 2500);
