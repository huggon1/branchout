  ...
  receive(c: Channel, message: BotMessage) {
    ...
    if (
      ...
      void this.reply(
        c,
        message.peer,
        "已绑定 Branchout。发送 GitHub 仓库或小红书分享链接即可解析。",
      );
      return;
    }
    ...
    void this.reply(
      ...
      message.peer,
      links.length
        ? `已接收 ${links.length} 条链接，正在排队解析。完整内容请在 Branchout 转发收件箱阅读。`
      ...
      while (this.pending.size && !this.closed) {
        ...
        if (result?.origin && !this.closed)
          await this.reply(
            ...
            result.origin.peer,
            `${result.material?.title || result.url}\n${result.state === "success" ? (result.material?.completeness === "partial" ? "部分解析完成" : "正文解析完成") : "解析失败，请在 Branchout 重试"}${result.summary ? `\nAI 摘要：${result.summary.slice(0, 1200)}` : result.state === "success" ? "\nAI 摘要暂不可用，原文已保存。" : ""}\n${result.url}`,
          );
      }
    } finally {
      this.draining = false;
