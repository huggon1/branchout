...
当前实现采用固定左侧工作空间目录和可伸缩工作区。默认侧栏为 232px，1100 宽度附近收紧为 210px；主区内边距为 36px 40px 64px，最大宽度为 1900px。侧栏只显示内容收集、项目理解、素材探索和 Feed 创作四个顶层工作空间，当前项原位展开直接子页面；连接与模型位于底部作为独立设置。这些是现有外壳尺寸，不是对未来页面构图的授权限制。
...
在 1250px 以下，侧栏为 210px，主区横向留白为 24px，顶部为 32px，通用面板内边距为 21px；连接设置在 1150px 以下收紧目录和内边距。1000px 以下生成页转为单列、编排取消粘性定位，部分表格辅助列隐藏。这些是窗口收缩策略；首版验收仍为桌面，尺寸与页面职责见 [页面设计入口](docs/design/README.md)。
...
工作空间目录使用图标、名称和一句职责说明，选中态为淡绿底和加深字重；直接子页面以细线缩进展开，当前页面使用白底强调。主区用工作空间位置线索和横向行动带说明资产、依赖与下一步，避免增加空洞总览页。设置入口靠近侧栏底部。连接目录采用图标列与文字列，名称允许折行，使用中状态置于文字下方，避免挤压名称。
